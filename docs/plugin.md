...
- `systemApi.comfyui.testConnection({ baseUrl })`（兼容接口）
- `systemApi.comfyui.generateAndAttach({ baseUrl, workflow, prompt, promptNodeId?, promptInputName?, conversationId, messageId })`（兼容接口）
- `systemApi.imageGeneration.testConnection({ provider: "comfyui", baseUrl })`
- `systemApi.imageGeneration.generateAndAttach({ provider: "comfyui", baseUrl, workflow, prompt, promptNodeId?, promptInputName?, conversationId, messageId })`
- `systemApi.imageGeneration.executeTask({ create, poll, conversationId, messageId })` 提供通用异步图片任务桥接。插件负责声明创建请求、轮询请求、任务 ID/状态/结果 JSON Pointer 路径及终态，Rust 只负责权限校验、HTTP 执行、轮询、图片下载和消息附件持久化。
...
- 对话标题栏可通过 `contributions.actions` + `renderAction()` 注入小型操作按钮，例如手动触发优化流程；消息悬浮工具栏使用 `location: "conversation.message-actions"`，上下文包含 `conversationId`、`messageId`、`messageType` 和 `messageContent`；
- ComfyUI 调用走 Rust `reqwest` 桥接，不依赖 WebView `fetch`。插件必须声明 `network.comfyui`；附加图片还需声明 `conversation.write`。地址仅允许无凭据的 `http/https` URL，生成结果会作为图片附件绑定到目标 assistant 消息并触发会话刷新。`promptNodeId` 和 `promptInputName` 用于指定 workflow 中写入提示词的节点和字符串参数，默认分别为 `57:27` 与 `text`；其它默认 workflow 节点和参数保持不变；
- `chat.afterResponseCompleted` 事件上下文包含 `assistantMessageId`，普通 Chat 与 ACP 都会提供该字段，插件应使用它精确定位完成的 assistant 消息；
