    ...
    pub fn new(app_handle: &tauri::AppHandle) -> Result<Self, DbErr> {
        ...
        if let Some(ds_state) = app_handle.try_state::<crate::DataStorageState>() {
            ...
            if let Some((dsn, backend)) = build_remote_dsn(&flat) {
                url = dsn;
                match backend {
                    DatabaseBackend::Postgres => debug!("Artifacts DB using remote PostgreSQL"),
                    DatabaseBackend::MySql => debug!("Artifacts DB using remote MySQL"),
                    _ => debug!("Artifacts DB using local SQLite"),
                }
            } else {
                debug!("Artifacts DB using local SQLite (no remote config or incomplete)");
            }
        }
